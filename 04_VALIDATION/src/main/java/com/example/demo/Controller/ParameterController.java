...
public class ParameterController {
    ...
    public void paramHandler_1(@RequestParam(name="username",required=false) String name){
        //요청파라미터를 받기 위한 어노테이션  / required=false == 파라미터의 필수 유무를 지정
//      public void paramHandler_1(@RequestParam String name)
    ...
    //@requestParam : 동기요청 파라미터 처리 (기본약속대로 처리하는 방식)
    //                html form 기반 전달되는 파라미터를 처리
    //                JS의 form-data도 받기 가능, Json Type 받기 불가능
    //@RequestBody  : 비동기요청 파라미터 처리
    //                json,filedata등 전달되는 파라미터 처리(html form 처리가능)
    @PostMapping("/p04")
    ...
    @PostMapping("/p05")
    //required=false,default="기본값"
    public void paramHandler_5(@RequestParam(name="username",required=false,defaultValue = "버섯등장") String name){
    ...
    @GetMapping("/p07")
    //@ModelAttribute dto 파라미터로 받아오기 / 생략 가능 (기본)
...
//        session.setAttribute("dto",dto);
    ...
    public String redirect_init_handler(Model model, RedirectAttributes redirectAttributes){
        ...
        redirectAttributes.addFlashAttribute("r_init2","r_init2_value");//session 속성

...
@Configuration
@EnableWebSecurity //시큐리티 설정 파일 어노테이션
public class SecurityConfig {
    ...
    protected SecurityFilterChain configure(HttpSecurity http) throws Exception {
        ...
        //csrf 비활성화 코드 (비활성화하지않으면 logout 요청은 기본적으로 POST방식을 따름
...
//따라서 평문 비밀번호를 테스트용으로 간단하게 쓰려면 {noop}을 붙여야 합니다
    ...
    //패스워드 암호화작업(해시값생성)에 사용되는 Bean (->userController에서 사용)
    @Bean
